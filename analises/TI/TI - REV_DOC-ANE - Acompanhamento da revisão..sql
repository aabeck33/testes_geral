...
((rev.fgcurrent = 2) and (doc.fgstatus in (1, 3, 5,8) and rev.cdrevision = (select max(cdrevision) from dcdocrevision where CDDOCUMENT = rev.cddocument))))
and (select dca.vlvalue from dcdocumentattrib dca where dca.cdattribute = 235 and dca.cdrevision = rev.cdrevision) in (202227168)
