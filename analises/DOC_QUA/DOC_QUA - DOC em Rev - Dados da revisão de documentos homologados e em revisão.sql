select cat.idcategory, cat.nmcategory, rev.iddocument, rev.nmtitle, gnrev.idrevision
...
, stag.dtdeadline, stag.dtapproval,qtdeadline
...
where doc.fgstatus in (1,2,3,5) and rev.cdrevision = (select max(revm.cdrevision) from dcdocrevision revm where revm.cddocument = doc.cddocument)
