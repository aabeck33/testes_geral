...
where plano.cdgenactivity = stpl.cdactionplan and wf.idprocess like 'SGEQ%' order by wf.idprocess) as idprocess
, (select top 1 wf.nmprocess
from gnassocactionplan stpl
inner JOIN GNACTIVITY GNP on stpl.cdassoc = GNP.cdassoc
inner join wfprocess wf on wf.CDGENACTIVITY = gnp.CDGENACTIVITY
where plano.cdgenactivity = stpl.cdactionplan and wf.idprocess like 'SGEQ%' order by wf.idprocess) as nmprocess
...
where plano.cdgenactivity = stpl.cdactionplan and wf.idprocess like 'SGEQ%' FOR XML PATH('')), 4, 4000)), '') as cliente
...
where aprov.cdapprov = max_cycle.cdapprov and plano.CDACTIONPLANTYPE = 217
