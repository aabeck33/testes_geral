select distinct sum(quantidade) as qtdtot, sum(case when situacao = 'Trained' then 1 else 0 end) as qtdok, sum(case when situacao = 'Pending' then 1 else 0 end) as qtdpend, sum(case when situacao = 'Waiting Training (over than 60 days)' then 1 else 0 end) as qtdathea, sum(case when situacao = 'Waiting Training - Waiting SOP to be effective' then 1 else 0 end) as qtdath, sum(case when situacao = 'Waiting Training (within 60 days)' then 1 else 0 end) as qtdat
...
, round(((Cast(sum(case when situacao = 'Waiting Training (over than 60 days)' then 1 else 0 end) AS NUMERIC(10,4))*100)/Cast(sum(quantidade) AS NUMERIC(10,4))),2) as pathea, round(((Cast(sum(case when situacao = 'Waiting Training - Waiting SOP to be effective' then 1 else 0 end) AS NUMERIC(10,4))*100)/Cast(sum(quantidade) AS NUMERIC(10,4))),2) as path
...
, case when (coalesce((select case when tu1.fgresult = 1 then 'Approved' when tu1.fgresult = 2 then 'Disapproved' end
              ...
              where tr1.cdcourse = co.cdcourse and ((revc.cdrevision is not null and trev1.cdrevision = revc.cdrevision) or (revc.cdrevision is null and trev1.cdrevision is null)) and
				tr1.fgstatus >= 5 and tr1.FGCANCEL <> 1 and tr1.cdtrain = (select max(tr2.cdtrain) as cdtrain
                                                        ...
                                                        where tr2.cdcourse = co.cdcourse and ((revc.cdrevision is not null and trev2.cdrevision = revc.cdrevision) or (revc.cdrevision is null and trev2.cdrevision is null)) and tr2.fgstatus >= 5 and tr2.FGCANCEL <> 1)), 'Not Rated') = 'Approved')
       then 'Trained'
