select distinct sum(quantidade) as qtdtot, sum(case when situacao = 'Trained' then 1 else 0 end) as qtdok, sum(case when situacao = 'Pending' then 1 else 0 end) as qtdpend, sum(case when situacao = 'Waiting Training (over than 60 days)' then 1 else 0 end) as qtdathea, sum(case when situacao = 'Training Done - Waiting SOP to be effective' then 1 else 0 end) as qtdath, sum(case when situacao = 'Waiting Training (within 60 days)' then 1 else 0 end) as qtdat
...
, round(((Cast(sum(case when situacao = 'Waiting Training (over than 60 days)' then 1 else 0 end) AS NUMERIC(10,4))*100)/Cast(sum(quantidade) AS NUMERIC(10,4))),2) as pathea, round(((Cast(sum(case when situacao = 'Training Done - Waiting SOP to be effective' then 1 else 0 end) AS NUMERIC(10,4))*100)/Cast(sum(quantidade) AS NUMERIC(10,4))),2) as path
, round(((Cast(sum(case when situacao = 'Waiting Training (within 60 days)' then 1 else 0 end) AS NUMERIC(10,4))*100)/Cast(sum(quantidade) AS NUMERIC(10,4))),2) as pat
...
, case when (coalesce((select case when tu1.fgresult = 1 then 'Approved' when tu1.fgresult = 2 then 'Disapproved' end
              ...
              where tr1.cdcourse = co.cdcourse and ((revc.cdrevision is not null and trev1.cdrevision = revc.cdrevision) or (revc.cdrevision is null and trev1.cdrevision is null)) and
				tr1.fgstatus >= 5 and tr1.FGCANCEL <> 1 and tr1.cdtrain = (select max(tr2.cdtrain) as cdtrain
                                                        ...
                                                        where tr2.cdcourse = co.cdcourse and ((revc.cdrevision is not null and trev2.cdrevision = revc.cdrevision) or (revc.cdrevision is null and trev2.cdrevision is null)) and tr2.fgstatus >= 5 and tr2.FGCANCEL <> 1)), 'Não avaliado') = 'Approved')
       then 'Trained'
                ...
                inner join gnrevision gnrevi on gnrevi.cdrevision = revi.cdrevision
                inner join GNREVISIONSTAGMEM stag1 ON stag1.CDREVISION = revi.CDREVISION
                where revi.cdrevision = (select max(cdrevision) from dcdocrevision where cddocument = revc.cddocument) and stag1.fgstage = 3))) <= 60 then 'Training Done - Waiting SOP to be effective' else 'Waiting Training (over than 60 days)' end end
end Situacao
, 1 as quantidade
from aduser usr
