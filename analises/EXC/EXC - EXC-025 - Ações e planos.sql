    ...
    FROM
        (SELECT
            ...
            CASE 
                ...
                AND GNACT.CDACTIVITYOWNER IS NOT NULL THEN '#{200381}' 
                WHEN GNACT.FGSTATUS=4 
            ...
            CASE 
                WHEN (GNACT.CDCALENDAR IS NULL) THEN CAST(NULL AS VARCHAR(255)) 
                ELSE GNCAL.IDCALENDAR + ' - ' + GNCAL.NMCALENDAR 
            END AS IDNMCALENDAR,
            CASE 
            ...
            END AS IDNMTEAM,
            CASE 
                WHEN TMCPLAN.FGPLAN=1 THEN ADUSR.NMUSER 
                ELSE ADUSR2.NMUSER 
            END AS NMPLANRESP,
            ADUSR2.IDUSER AS IDACTIONPLANRESP,
            ADUSR2.NMUSER AS NMACTIONPLANRESP,
            CASE 
                WHEN (AD2.IDDEPARTMENT IS NOT NULL) THEN AD2.IDDEPARTMENT + ' - ' + AD2.NMDEPARTMENT 
                ELSE CAST(NULL AS VARCHAR(255)) 
            END AS IDNMDEPTUSERRESP,
            CASE 
                WHEN (AP2.IDPOSITION IS NOT NULL) THEN AP2.IDPOSITION + ' - ' + AP2.NMPOSITION 
                ELSE CAST(NULL AS VARCHAR(255)) 
            END AS IDNMPOSUSERRESP,
            CASE 
                WHEN GNAPPLAN.FGINCREMENTAL=1 THEN '#{103405}' 
                WHEN GNAPPLAN.FGINCREMENTAL=2 THEN '#{103404}' 
                ELSE CAST(NULL AS VARCHAR(255)) 
            END AS PLAN_APPROV_TYPE,
            CASE 
                WHEN GNAPEXEC.FGINCREMENTAL=1 THEN '#{103405}' 
                WHEN GNAPEXEC.FGINCREMENTAL=2 THEN '#{103404}' 
                ELSE CAST(NULL AS VARCHAR(255)) 
            END AS EXEC_APPROV_TYPE,
            GNAPPLAN.FGINCREMENTAL AS FGINC_PLAN_TYPE,
            GNAPEXEC.FGINCREMENTAL AS FGINC_EXEC_TYPE,
            GNAPPLAN.NRLASTCYCLE AS PLAN_APPROV_CYCLE,
            GNAPEXEC.NRLASTCYCLE AS EXEC_APPROV_CYCLE,
            1 AS QTD 
        FROM
        ...
        INNER JOIN
            GNACTIVITY GNACT 
                ON (
                    ...
                    AND GNACT.CDACTIVITYOWNER IS NULL
                ) 
        LEFT OUTER JOIN
            GNACTIONPLANTYPE GNACTTP 
                ON (
                    GNACTTP.CDACTIONPLANTYPE=TMCPLAN.CDACTIONPLANTYPE
                ) 
        LEFT OUTER JOIN
            ADUSER ADUSR 
                ON (
        ...
        LEFT OUTER JOIN
            (
                ...
                INNER JOIN
                    GNASSOCDOCUMENT 
                        ON GNASSOCDOCUMENT.CDASSOC=GNACTIVITY.CDASSOC 
                WHERE
                    GNACTIVITY.CDISOSYSTEM=174 
                GROUP BY
                    CDACTIVITYOWNER
            ) DOCCHILDASSOC 
                ON DOCCHILDASSOC.CDACTIVITYOWNER=GNACT.CDGENACTIVITY 
        LEFT OUTER JOIN
            ADUSERDEPTPOS ADUD2 
                ON (
                    ADUSR2.CDUSER=ADUD2.CDUSER 
                    AND ADUD2.FGDEFAULTDEPTPOS=1
                ) 
        LEFT OUTER JOIN
            ADDEPARTMENT AD2 
                ON (
                    ADUD2.CDDEPARTMENT=AD2.CDDEPARTMENT
                ) 
        LEFT OUTER JOIN
            ADPOSITION AP2 
                ON (
                    ADUD2.CDPOSITION=AP2.CDPOSITION
                ) 
        LEFT OUTER JOIN
            GNAPPROV GNAPPLAN 
                ON (
                    GNAPPLAN.CDAPPROV=GNACT.CDPLANROUTE 
                    AND GNAPPLAN.CDPROD=GNACT.CDPLANPRODROUTE
                ) 
        LEFT OUTER JOIN
            GNAPPROV GNAPEXEC 
                ON (
                    GNAPEXEC.CDAPPROV=GNACT.CDEXECROUTE 
                    AND GNAPEXEC.CDPROD=GNACT.CDPRODROUTE
                ) 
        LEFT OUTER JOIN
            GNCALENDAR GNCAL 
                ON (
                    GNACT.CDCALENDAR=GNCAL.CDCALENDAR
                ) 
        WHERE
            1=1 
            AND TMCPLAN.FGMODEL=2 
            AND EXISTS (
                SELECT
                    1 
                FROM
                    ADMINISTRATION 
                WHERE
                    COALESCE(TMCPLAN.FGOBJECT,-1) <> 9 /*sub*/
                UNION
                /*sub*/ ALL SELECT
                    1 
                FROM
                ...
                WHERE
                    ...
                    AND  (
                        NOT EXISTS (
                            ...
                            WHERE
                                GNACPLAPDI.CDACTIONPLAN=TMCPLAN.CDGENACTIVITY
                        ) 
                        OR EXISTS (
                            SELECT
                                1 
                        ...
                        WHERE
                            GNACTPDI.CDGENACTIVITY=TMCPLAN.CDGENACTIVITY 
                            AND (
                                EXISTS (
                                    SELECT
                                        ADT1.CDUSER 
        ...
        AND (
            (
                GNACT.IDACTIVITY LIKE '%0052%'
            ) 
            OR (
                GNACT.NMACTIVITY LIKE '%0052%'
            )
        ) 
        AND (
