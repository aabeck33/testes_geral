        ...
        CASE 
            ...
            OR GNACT.FGSTATUS=2 THEN CASE 
                ...
                OR TMCPLAN.FGIMMEDIATEACTION IS NULL) THEN CASE 
                    ...
                    getdate()) * 60) AS NUMERIC(10))) THEN '#{100899}' 
            ...
            OR GNACT.FGSTATUS=13 THEN CASE 
                ...
                WHERE
                    ...
                    WHERE
                        ...
                        AND VLPARAM=1) 
                    AND (GNACT.DTSTARTPLAN=<!%TODAY%>) 
                    AND (GNACT.QTTIMESTARTPLAN < GNACT.QTTIMESTART)) THEN '#{100899}' 
                ...
                END 
                WHEN (GNACT.DTSTART IS NOT NULL 
                AND GNACT.DTFINISH IS NOT NULL 
                AND ((SELECT
                    VLPARAM 
                FROM
                    ADPARAMS 
                WHERE
                    ...
                    AND (GNACT.QTTIMEFINISHPLAN < GNACT.QTTIMEFINISH)) THEN '#{100899}' 
                END 
...
(
    (
        TMCPLAN.CDACTIONPLANTYPE IN(
            <!%FUNC(
                com.softexpert.generic.parameter.InClauseBuilder, R05HRU5UWVBF, Q0RHRU5UWVBF, Q0RHRU5UWVBFT1dORVI=,, NTc=,
            )%>
        )
    )
) 
