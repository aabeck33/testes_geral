...
, classinic.tbs005 as classificini, catevent.tbs006 as catevento --classfin.tbs002 as classfim--
, catraiz.tbs007 as catcausaraiz
, classfin.tds055 as classfim
, areadec.tbs11 as AreaDeDeteccao
, areaocor.tbs11 as AreaDeOcorrencia
, subcat.tbs007 as SubCatDes
, cci.tbs005 as CCI 
, crid.tds001 as CRID
, form.tds075 as EquipamentoEnvolvido
, areaoca.tbs11 as AreaOcasionadora
, alcoa.tds054 as AtributoALCOA
, form.tds130 as CFRD
, form.tds075 as DDE
, form.tds049 as CSA
, case form.tds052 when 1 then 'Sim' when 2 then 'Não' end recorrente
, case form.tds127 when 1 then 'Alimento/Vitamínicos ou correlatos' when 3 then 'Medicamentos não-estéreis' when 2  then 'Medicamentos estéreis'  end DRA
, case form.tds163 when 1 then 'Sim' when 2 then 'Não' end ImpactoIntegridadeDados
...
from DYNtbs017 where OIDABCJVZ44ILEEKJM = form.oid FOR XML PATH('')), 4, 8000)), 'NA') as varchar(8000)) as CausaContribuinte
...
left join DYNtbs011 areadec on areadec.oid = form.OIDABC1MOABCE0S
left join DYNtbs006a subcat on subcat.oid = form.OIDABC8SS6ABJMOQZ1
left join DYNtbs005 cci on cci.oid = form.OIDABCCM0ABCSBB
left join DYNtds055 crid on crid.oid = form.OIDABC35KTYTUNJ348
left join DYNtbs011 areaoca on areaoca.oid = form.OIDABC3N8FX4QYR0ND
left join DYNtds010 alcoa on alcoa.oid = form.OIDABCQ9G6DQOKD8P4
left join DYNtds010 classfin on classfin.oid = form.OIDABCHBARQW3QRTLX
