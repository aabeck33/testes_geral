...
,idposition+' - '+ nmposition as position, revc.iddocument, gnrevc.idrevision, revc.iddocument +'-'+ gnrevc.idrevision as POP
, case when (coalesce((select case when tu1.fgresult = 1 then 'Aprovado' when tu1.fgresult = 2 then 'Reprovado' end
       ...
       then 'Treinado'
                ...
                inner join gnrevision gnrevi on gnrevi.cdrevision = revi.cdrevision
                inner join GNREVISIONSTAGMEM stag1 ON stag1.CDREVISION = revi.CDREVISION
                where revi.cdrevision = (select max(cdrevision) from dcdocrevision where cddocument = revc.cddocument) and stag1.fgstage = 4))) <= 60 then 'Aguardando Treinamento – Homologação' else 'Aguardando Treinamento – Homologação (em atraso)' end end
end Situacao
, revc.fgcurrent as FGCURRENT
, 1 as quantidade
from aduser usr
...
inner join adposition pos on pos.cdposition = rel0.cdposition and pos.fgposenabled = 1 and (pos.idposition like 'LB0090%')
...
inner join trcourse co on co.cdcourse = relc.cdcourse and co.fgenabled = 1 and co.cdcoursetype in (357)
